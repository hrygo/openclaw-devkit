...
> **⚠️ 挂载说明**：容器运行时配置通过 bind mount 存储在宿主机 `~/.openclaw/`。
>
> **宿主机 `~/.openclaw/` 仅作为初始化种子**，首次启动后不再使用。
>
> **配置方式**：
> - **方法一**：直接编辑宿主机 `~/.openclaw/openclaw.json`
> - **方法二**：使用 CLI 命令
> - **方法三**：进入容器编辑
...
│                     Docker 容器                             │
│                                                             │
│  /home/node/.openclaw/        ← Bind Mount (读写)          │
│  └── openclaw.json            ← 对应宿主机的 ~/.openclaw/  │
│  /home/node/.claude/          ← 命名卷 (Session/Memory)   │
│  /home/node/.global/          ← 命名卷 (工具链)            │
└─────────────────────────────────────────────────────────────┘
```

**关键点**：
- **实时共享**：`~/.openclaw/` 通过 bind mount 与容器实时同步，直接编辑即生效
- **Claude Code 持久化**：Session/Memory 存储在 `openclaw-claude-home` 命名卷，重建不丢失
- **修改方式**：
  - ✅ 宿主机编辑 `~/.openclaw/openclaw.json`
